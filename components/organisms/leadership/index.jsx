...
import styles from "./leadershipStyles";
        ...
        <div className={styles.grid}>
          {leaders.map((leader, i) => (
            <div key={i} className={styles.column}>
              ...
              {/* Quote Text */}
              <p className={styles.quoteText}>{leader.quote}</p>

              {/* Author Info */}
                <div className={styles.authorBlock}>
                  <h3 className={styles.authorName}>{leader.name}</h3>
                  <p className={styles.authorTitle}>{leader.designation} <span className="block">{leader.company}</span></p>
                </div>

                {/* Image with Dot Pattern (Absolute at bottom right) */}
                <div className={`${styles.imageWrapper} absolute bottom-0 right-0`}>
                  <CustomImage
